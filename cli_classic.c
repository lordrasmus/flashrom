...
static void show_status_regs( struct flashctx *fill_flash ) {

	printf("\nshow_status_regs\n\n");
	if ( fill_flash->chip->print_status_register != 0 ) {
		fill_flash->chip->print_status_register( fill_flash );
	}else{
		printf("  show_status_regs not supported on %s %s\n\n", fill_flash->chip->vendor, fill_flash->chip->name );

		const struct flashchip *chip;
		printf("  only supported on :\n");
		for (chip = flashchips ; chip && chip->name; chip++) {
			if ( chip->print_status_register != 0 ){
